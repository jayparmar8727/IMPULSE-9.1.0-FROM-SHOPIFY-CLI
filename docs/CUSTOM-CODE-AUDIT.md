...
| **Non-text brass `#bc843f` / `#d8ae82`** (borders, dividers, accent lines/dashes, button-hover bg, dots, rating stars, all dark-bg accents) | Brass is the intentional brand accent; non-text + stars pass (or are exempt from) AA, and brass/tan on dark already meets AA | `kw-section-conventions` memory. **UPDATE 2026-06-07:** brass *small text on light* was reconciled to accessible amber `#9a6326` (AA) — see BRAND.md §8 / `assets/kw-tokens.css` `--Br-aa`. Only non-text brass is unchanged. |
